...
invoiceRoutes.get("/", async (c) => {
  ...
  return c.json({ invoices: result.results, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } });
});
...
invoiceRoutes.get("/:id/pdf", async (c) => {
  ...
  const invoice = await c.env.DB.prepare("SELECT invoice_number FROM invoices WHERE id = ? AND is_deleted = 0").bind(id).first<{ invoice_number: string }>();
  return new Response(pdf, { status: 200, headers: { "Content-Type": "application/pdf", "Content-Disposition": `inline; filename="${invoice?.invoice_number ?? `invoice-${id}`}.pdf"`, "Cache-Control": "private, no-store" } });
});
...
invoiceRoutes.post("/", async (c) => {
  ...
  } catch (error) { return c.json({ error: error instanceof Error ? error.message : "Failed to create invoice" }, 400); }
});

invoiceRoutes.post("/:id/cancel", requireAdmin, async (c) => {
  const id = Number(c.req.param("id"));
  if (!Number.isInteger(id)) return c.json({ error: "Invalid invoice ID" }, 400);
