...
export function createPdfPage(texts: PdfText[], lines: PdfLine[] = [], images: PdfImage[] = []): string {
  ...
  for (const item of texts) {
    ...
    commands.push(`BT ${font} ${size} Tf 1 0 0 1 ${item.x} ${item.y} Tm (${escapePdfText(item.text)}) Tj ET`);
  }
...
export function buildPdf(pages: string[], image?: { bytes: Uint8Array; width: number; height: number }): Uint8Array {
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    const imageResource = image ? ` /XObject << /Im1 ${3 + pages.length * 2 + 2} 0 R >>` : "";
    objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /ProcSet [/PDF /Text${image ? " /ImageC" : ""}] /Font << /F1 ${3 + pages.length * 2} 0 R /F2 ${4 + pages.length * 2} 0 R >>${imageResource} >> /Contents ${contentObject} 0 R >>`);
    const stream = pages[index];
    const length = encoder.encode(stream).byteLength;
  ...
  if (image) {
    ...
    objects.push(`<< /Type /XObject /Subtype /Image /Width ${image.width} /Height ${image.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter [/ASCII85Decode /DCTDecode] /Length ${encoder.encode(encoded).byteLength} >>\nstream\n${encoded}\nendstream`);
    void imageObject;
  }
